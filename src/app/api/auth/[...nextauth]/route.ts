    ...
    WordPressProvider({
      ...
      token: {
        ...
        async request({ params }) {
          ...
          if (!response.ok) {
            throw new Error(
              `WordPress Token Exchange Failed: ${await response.text()}`
            );
          }
    ...
    async jwt({ token, user, account, profile }) {
      if (user) {
        ...
        if (profile && account?.provider === "google") {
          token.name = `${(profile as any).given_name} ${
        ...
        try {
          ...
          const response = await fetch(
            `${process.env.NEXT_PUBLIC_BACKEND_URL}/authLogin`,
            {
              method: "POST",
    ...
    async session({ session, token }) {
      if (session?.user) {
        ...
        session.user.backendToken = token.backendToken
          ? token.backendToken
          : "sample-debug-token"; // Add backend token to session
      }
