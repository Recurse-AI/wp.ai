...
const AuthService = {
  ...
  register: (userData: UserRegistration) =>
    apiPost<{ message: string; user: UserProfile }>(
      ApiPaths.REGISTER,
      userData
    ),
  ...
  logout: () => {
    ...
    return apiPost<{ detail: string }>(ApiPaths.LOGOUT, {
      refresh_token: refreshToken,
    });
  },

  // Social authentication
  googleAuth: (email: string, image?: string) =>
    apiPost<TokenResponse>(ApiPaths.GOOGLE_AUTH, {
      email,
      image,
      provider: "google",
    }),
  ...
  facebookAuth: (accessToken: string) =>
    apiPost<TokenResponse>(ApiPaths.FACEBOOK_AUTH, {
      access_token: accessToken,
    }),

  githubAuth: (email: string) =>
    apiPost<TokenResponse>(ApiPaths.GITHUB_AUTH, { email, provider: "github" }),

  wordpressAuth: (email: string) =>
    apiPost<TokenResponse>(ApiPaths.WORDPRESS_AUTH, {
      email,
      provider: "wordpress",
    }),
  ...
  requestPhoneVerification: (phone: string) =>
    apiPost<{ message: string }>(ApiPaths.PHONE_VERIFICATION, {
      phone_number: phone,
    }),
  ...
  confirmPasswordReset: (data: ResetPasswordConfirmRequest) =>
    apiPost<{ message: string }>(
      ApiPaths.RESET_PASSWORD_CONFIRM(data.uid, data.token),
      {
  ...
  getUsers: (page = 1, pageSize = 10) =>
    apiGet<{ count: number; results: UserProfile[] }>(
      `${ApiPaths.USERS}?page=${page}&page_size=${pageSize}`
    ),

  getUser: (id: number) => apiGet<UserProfile>(ApiPaths.USER_DETAIL(id)),
