...
const LazyWordPressPlayground = dynamic(() => import('../agent-comp/WordPressPlayground'), {
    loading: () => <div className="text-sm text-gray-500 p-4 text-center">Loading WordPress Playground...</div>,
    ssr: false
});
...
const AgentModeInterface: React.FC<AgentModeInterfaceProps> = ({
    ...
    const [activeTab, setActiveTab] = useState<'editor' | 'preview' | 'playground'>('editor');
    ...
    const toggleSidebar = useCallback(() => {
        ...
        if (isClient) {
            toast.success(newSidebarState ? "Exited fullscreen mode" : "Entered fullscreen mode", {
            ...
            <div className="flex flex-1 overflow-hidden relative">
                {!fullscreenMode && (
                    <Resizable
                        ...
                        maxWidth="500px"
                        className={`border-r border-gray-200 dark:border-gray-700 transition-all duration-300 ${sidebarOpen ? 'flex flex-col' : 'w-0 overflow-hidden'}`}
                    >
                        {sidebarOpen && (
                            <Tabs defaultValue="messages" className="w-full h-full">
                                ...
                                <TabsContent value="messages" className="flex-1 overflow-auto p-2 custom-scrollbar">
                                    <div className={`h-full overflow-y-auto p-4 ${theme === 'dark' ? 'bg-gray-900/50' : 'bg-white'} rounded-md`}>
                                        {displayMessages.length > 0 ? displayMessages.map((msg, index) => (
                                            <AgentMessage key={msg.id || index} content={msg.content} isUser={msg.role === 'user'} isLatestMessage={index === displayMessages.length - 1} />
                                        )) : (
                                ...
                                <TabsContent value="files" className="flex-1 overflow-auto p-0 custom-scrollbar">
                                    <FileExplorer onFileSelect={handleFileSelect} onFilesChange={setCurrentFiles} />
                                </TabsContent>
                            </Tabs>
                        )}
                    </Resizable>
                ...
                {isClient && (
                    <button onClick={toggleSidebar} data-tooltip-id="sidebar-tooltip" data-tooltip-content={sidebarOpen ? "Hide sidebar (enter fullscreen)" : "Show sidebar (exit fullscreen)"} className={`absolute left-0 top-1/2 transform -translate-y-1/2 ${fullscreenMode ? 'bg-purple-500 hover:bg-purple-600 text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300'} p-1.5 rounded-r-md border border-l-0 ${fullscreenMode ? 'border-purple-600 dark:border-purple-800 shadow-md' : 'border-gray-200 dark:border-gray-700'} z-10 transition-all duration-300 hover:scale-105`}>
                ...
                <div className="flex-1 flex flex-col overflow-hidden transition-all duration-500">
                    <Tabs defaultValue="editor" className="w-full h-full flex flex-col" onValueChange={(value) => setActiveTab(value as any)}>
                        <TabsList className="w-full flex justify-start border-b border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-800">
                            ...
                            <TabsTrigger value="playground" className="flex items-center gap-1 py-2 relative">
                        ...
                        <TabsContent value="editor" className="flex-1 overflow-hidden p-0 vscode-editor-tab">
                            {selectedFile ? (
                                <CodeEditor
                                    ...
                                    onChange={(newCode) => setSelectedFile({ ...selectedFile, content: newCode })}
                        ...
                        <TabsContent value="playground" className="flex-1 overflow-auto p-0 vscode-preview-tab">
