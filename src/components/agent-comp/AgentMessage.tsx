...
const AgentMessage: React.FC<AgentMessageProps> = ({
  ...
  const isReactCode = (code: string): boolean => {
    ...
    if ((code.includes('<') && code.includes('>')) && 
        ...
        (code.includes('className=') || code.includes('onClick=') || 
         code.includes('props') || code.includes('style=') || 
         code.includes('children'))) {
      return true;
    }
    
    // Check for React component patterns
    if (/function\s+[A-Z][a-zA-Z]*\s*\(/g.test(code) || 
  ...
  const detectLanguageFromFileName = (fileName: string): string | null => {
    ...
    if (/[A-Z][a-zA-Z]*\.jsx?$/.test(fileName) || 
  ...
  const renderContentWithCodeBlocks = (text: string) => {
    ...
    while ((match = codeBlockRegex.exec(text)) !== null) {
      ...
      const fileNameLang = fileName ? detectLanguageFromFileName(fileName) : null;
      ...
      const isJsonContent = !isJsxCode && (() => {
        try {
          // Check if it starts with { or [ and try parsing it
          if ((codeContent.trim().startsWith('{') || codeContent.trim().startsWith('[')) &&
              (codeContent.trim().endsWith('}') || codeContent.trim().endsWith(']'))) {
      ...
      if (fileNameLang) {
        ...
        detectedLang = specifiedLang === 'tsx' || specifiedLang === 'typescript' ? 'tsx' : 'jsx';
      ...
      parts.push(
        <div key={`code-${match.index}`} className="relative my-3 rounded-md overflow-hidden code-block-wrapper">
          ...
          <div 
            ...
            style={{
              top: "8px",
              left: "8px",
              background: theme === "dark" ? "rgba(40, 40, 40, 0.9)" : "rgba(240, 240, 240, 0.9)",
          ...
          <div className={`pt-6 ${
            isJsxCode ? 'jsx-code-block' : 
            detectedLang === 'json' ? 'json-code-block' : 
            ''
          }`}>
            <CodeSyntaxHighlighter
              ...
              className={`w-full vscode-theme ${
                isJsxCode ? 'syntax-jsx' : 
                detectedLang === 'json' ? 'syntax-json' : 
                ''
  ...
  return (
    ...
    >
      ...
      >
        <Avatar className={`rounded-full overflow-hidden w-8 h-8 mt-1 flex-shrink-0 ${
          isUser 
            ? 'bg-blue-500' 
            : 'bg-purple-600'
        }`}>
        ...
        >
          {isClient ? (
            processedContent.map((block, index) => {
              if (block.type === 'code') {
                return (
                  <div key={index} className="my-2 overflow-hidden rounded-md">
                    <CodeSyntaxHighlighter
                      code={block.content}
            ...
            // Server-side rendering placeholder with the same structure
      ...
      <style jsx global>{`
        ...
        .code-block-wrapper pre {
          ...
          background: ${theme === 'dark' ? '#1e1e1e !important' : '#ffffff !important'};
        ...
        .code-block-wrapper .react-syntax-highlighter-line-number {
          color: ${theme === 'dark' ? '#858585 !important' : '#a0a0a0 !important'};
          background: ${theme === 'dark' ? '#1e1e1e !important' : '#ffffff !important'};
          border-right: 1px solid ${theme === 'dark' ? '#333333 !important' : '#e4e4e4 !important'};
          font-family: Consolas, Monaco, 'Andale Mono', 'Ubuntu Mono', monospace !important;
